...
const UserFollower: React.FC<UserFollowerProps> = ({
  ...
  const fetchFollowData = async () => {
    ...
    try {
      const [followersResponse, followingResponse] = await Promise.all([
        fetch(`http://localhost:8090/api/v1/follows/followers/${userId}`),
        fetch(`http://localhost:8090/api/v1/follows/following/${userId}`),
      ]);
  ...
  const handleFollowToggle = async (targetUserId: string) => {
    try {
      ...
      const response = await fetch(
        `http://localhost:8090/api/v1/follows/${endpoint}`,
        {
  ...
  // isOpen이 true로 변경될 때마다 데이터 새로 불러오기
  useEffect(() => {
    if (isOpen) {
