            ...
            {bloggers.map((blogger: Blogger, index: number) => (
              <div key={index} className="flex flex-col items-center">
                ...
                <span className="text-xs text-gray-500">
                  팔로워{" "}
                  {blogger.followerCount
                    ? blogger.followerCount.toLocaleString()
                    : "0"}
                </span>
              </div>
            ))}
            ...
            {Array.isArray(topPosts) &&
              topPosts.map((post) => (
                <Link href={`/boards/${post.id}`} key={post.id}>
                  <div className="rounded-lg overflow-hidden shadow-sm border border-gray-100 hover:shadow-md transition-shadow cursor-pointer">
        ...
        <section className="mb-8">
          ...
          <div className="space-y-4">
            {realtimePosts.map((post) => (
              <Link href={`/boards/${post.id}`} key={post.id}>
                <div className="flex gap-4 border-b pb-4 hover:bg-gray-50 transition-colors cursor-pointer">
