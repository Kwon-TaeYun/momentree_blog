...
export default function BoardDetail() {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      try {
        // 쿠키 기반 인증
        const response = await fetch("http://localhost:8090/api/v1/members/me", {
          credentials: "include",
          headers: token ? {
    ...
    if (currentUser !== null || !loading) {
      const fetchPost = async () => {
        ...
        try {
          ...
          const response = await fetch(`http://localhost:8090/api/v1/boards/${boardIdNumber}`, {
            credentials: "include",
            headers
          });
          ...
          if (currentUser && data.likeUsers && Array.isArray(data.likeUsers)) {
            ...
            // 객체 배열인 경우 현재 사용자의 ID가 포함되어 있는지 확인
              ...
              setIsLiked(data.likeUsers.some((user: any) => 
          ...
          setError(error instanceof Error ? error.message : "게시글을 불러오는 데 실패했습니다.");
        } finally {
          setLoading(false);
        }
      };
  ...
  const handleLikeToggle = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:8090/api/v1/boards/${boardIdNumber}/likes`, {
        method: method,
        headers,
        credentials: "include",
      });
      ...
      if (response.ok) {
        ...
        alert(`좋아요 처리 중 오류가 발생했습니다. ${errorData?.message || ''}`);
      }
    } catch (error) {
  ...
  const handleCommentSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(`http://localhost:8090/api/v1/boards/${boardIdNumber}/comments`, {
        method: "POST",
        headers,
        credentials: "include",
  ...
  if (!post) {
    return <div className="text-center py-20 text-gray-500">게시글을 찾을 수 없습니다.</div>;
  }
     ...
     {post.photos && post.photos.length > 0 && (
        <div className="mb-6 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
          {post.photos.map((photoUrl, index) => (
            <img key={index} src={photoUrl} alt={`게시글 이미지 ${index + 1}`} className="rounded-xl w-full h-auto object-cover" />
          ))}
        </div>
      )}
        ...
        {isAuthenticated ? (
          ...
          <div className="bg-gray-50 p-4 rounded-lg text-center">
            <p className="text-gray-600">
              댓글을 작성하려면{" "}
              <a href="/members/login" className="text-[#2c714c] hover:underline">
                로그인
              </a>{" "}
              이 필요합니다.
            </p>
